# Contributing to aov-lab

Thanks for wanting to make aov-lab better. Whether you're fixing a typo in a skill prompt or building an entirely new workflow, this guide will get you up and running fast.

## Quick start

aov-lab skills are Markdown files that Claude Code discovers from a `skills/` directory. Normally they live at `~/.claude/skills/aov-lab/` (your global install). But when you're developing aov-lab itself, you want Claude Code to use the skills *in your working tree* — so edits take effect instantly without copying or deploying anything.

That's what dev mode does. It symlinks your repo into the local `.claude/skills/` directory so Claude Code reads skills straight from your checkout.

```bash
git clone <repo> && cd aov-lab
...
Contributor mode turns aov-lab into a self-improving tool. Enable it and Claude Code
will periodically reflect on its aov-lab experience — rating it 0-10 at the end of
each major workflow step. When something isn't a 10, it thinks about why and files
a report to `~/.aov-lab/contributor-logs/` with what happened, repro steps, and what
would make it better.

```bash
~/.claude/skills/aov-lab/bin/aov-lab-config set aov_lab_contributor true
```

The logs are for **you**. When something bugs you enough to fix, the report is
already written. Fork aov-lab, symlink your fork into the project where you hit
the issue, fix it, and open a PR.

### The contributor workflow

1. **Use aov-lab normally** — contributor mode reflects and logs issues automatically
2. **Check your logs:** `ls ~/.aov-lab/contributor-logs/`
3. **Fork and clone aov-lab** (if you haven't already)
4. **Symlink your fork into the project where you hit the bug:**
   ```bash
   # In your core project (the one where aov-lab annoyed you)
   ln -sfn /path/to/your/aov-lab-fork .claude/skills/aov-lab
   cd .claude/skills/aov-lab && bun install && bun run build
   ```
5. **Fix the issue** — your changes are live immediately in this project
6. **Test by actually using aov-lab** — do the thing that annoyed you, verify it's fixed
7. **Open a PR from your fork**

This is the best way to contribute: fix aov-lab while doing your real work, in the
project where you actually felt the pain.

### Session awareness

When you have 3+ aov-lab sessions open simultaneously, every question tells you which project, which branch, and what's happening. No more staring at a question thinking "wait, which window is this?" The format is consistent across all 15 skills.

## Working on aov-lab inside the aov-lab repo

When you're editing aov-lab skills and want to test them by actually using aov-lab
...
```
aov-lab/                          <- your working tree
├── .claude/skills/              <- created by dev-setup (gitignored)
│   ├── aov-lab -> ../../         <- symlink back to repo root
│   ├── review -> aov-lab/review
│   ├── ship -> aov-lab/ship
...
When E2E tests run, they produce machine-readable artifacts in `~/.aov-lab-dev/`:
...
Artifacts are never cleaned up — they accumulate in `~/.aov-lab-dev/` for post-mortem debugging and trend analysis.
...
aov-lab generates SKILL.md files for two hosts: **Claude** (`.claude/skills/`) and **Codex** (`.agents/skills/`). Every template change needs to be generated for both.
...
| Aspect | Claude | Codex |
|--------|--------|-------|
| Output directory | `{skill}/SKILL.md` | `.agents/skills/aov-lab-{skill}/SKILL.md` |
| Frontmatter | Full (name, description, allowed-tools, hooks, version) | Minimal (name + description only) |
| Paths | `~/.claude/skills/aov-lab` | `~/.codex/skills/aov-lab` |
...
4. Commit both `{skill}/SKILL.md` and `.agents/skills/aov-lab-{skill}/SKILL.md`
...
- **Dev mode shadows your global install.** Project-local skills take priority over `~/.claude/skills/aov-lab`. `bin/dev-teardown` restores the global one.
...
**This is the recommended way to develop aov-lab.** Symlink your aov-lab checkout
into the project where you actually use it, so your changes are live while you
do real work:

```bash
# In your core project
ln -sfn /path/to/your/aov-lab-checkout .claude/skills/aov-lab
cd .claude/skills/aov-lab && bun install && bun run build
```

Now every aov-lab skill invocation in this project uses your working tree. Edit a
...
```bash
rm .claude/skills/aov-lab
```

Claude Code falls back to `~/.claude/skills/aov-lab/` automatically.

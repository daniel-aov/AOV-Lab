// aov-lab telemetry-ingest edge function
// Validates and inserts a batch of telemetry events.
// Called by bin/aov-lab-telemetry-sync.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const event of events) {
      // Required fields
      if (!event.ts || !event.aov_lab_version || !event.os || !event.outcome) {
      ...
      rows.push({
        ...
        aov_lab_version: String(event.aov_lab_version).slice(0, 20),

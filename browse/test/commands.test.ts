...
describe('CLI server script resolution', () => {
  test('prefers adjacent browse/src/server.ts for compiled project installs', () => {
    const root = fs.mkdtempSync('/tmp/aov-lab-cli-');
    const execPath = path.join(root, '.claude/skills/aov-lab/browse/dist/browse');
    const serverPath = path.join(root, '.claude/skills/aov-lab/browse/src/server.ts');

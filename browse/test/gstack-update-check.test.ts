/**
 * Tests for bin/aov-lab-update-check bash script.
 *
...
const SCRIPT = join(import.meta.dir, '..', '..', 'bin', 'aov-lab-update-check');

let aov-labDir: string;
let stateDir: string;
...
function run(extraEnv: Record<string, string> = {}, args: string[] = []) {
  const result = Bun.spawnSync(['bash', SCRIPT, ...args], {
    env: {
      ...process.env,
      GSTACK_DIR: aov-labDir,
      GSTACK_STATE_DIR: stateDir,
      GSTACK_REMOTE_URL: `file://${join(aov-labDir, 'REMOTE_VERSION')}`,
      ...extraEnv,
    },
    stdout: 'pipe',
    stderr: 'pipe',
...
beforeEach(() => {
  aov-labDir = mkdtempSync(join(tmpdir(), 'aov-lab-upd-test-'));
  stateDir = mkdtempSync(join(tmpdir(), 'aov-lab-state-test-'));
  // Link real aov-lab-config so update_check config check works
  const binDir = join(aov-labDir, 'bin');
  mkdirSync(binDir);
  symlinkSync(join(import.meta.dir, '..', '..', 'bin', 'aov-lab-config'), join(binDir, 'aov-lab-config'));
});

afterEach(() => {
  rmSync(aov-labDir, { recursive: true, force: true });
...
describe('aov-lab-update-check', () => {
  ...
  test('exits 0 with no output when VERSION file is empty', () => {
    writeFileSync(join(aov-labDir, 'VERSION'), '');
    const { exitCode, stdout } = run();
    expect(exitCode).toBe(0);
    expect(stdout).toBe('');
  });
  ...
  test('outputs JUST_UPGRADED and deletes marker', () => {
    writeFileSync(join(aov-labDir, 'VERSION'), '0.4.0\n');
  ...
  test('exits silently when cache says UP_TO_DATE and is fresh', () => {
    writeFileSync(join(aov-labDir, 'VERSION'), '0.3.3\n');
  ...
  test('re-checks when UP_TO_DATE cache version does not match local', () => {
    ...
    writeFileSync(join(aov-labDir, 'REMOTE_VERSION'), '0.5.0\n');
  ...
  test('re-checks when local version does not match cached old version', () => {
    ...
    writeFileSync(join(aov-labDir, 'REMOTE_VERSION'), '0.4.0\n');
  ...
  test('writes UP_TO_DATE cache when versions match', () => {
    writeFileSync(join(aov-labDir, 'VERSION'), '0.3.3\n');
    writeFileSync(join(aov-labDir, 'REMOTE_VERSION'), '0.3.3\n');
  ...
  test('treats invalid remote response as up to date', () => {
    writeFileSync(join(aov-labDir, 'VERSION'), '0.3.3\n');
    writeFileSync(join(aov-labDir, 'REMOTE_VERSION'), '<html>404 Not Found</html>\n');
  ...
  test('exits 0 with real project VERSION and unreachable remote', () => {
    ...
    writeFileSync(join(aov-labDir, 'VERSION'), version + '\n');

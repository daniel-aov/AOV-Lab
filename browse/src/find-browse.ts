/**
 * find-browse — locate the aov-lab browse binary.
 *
...
export function locateBinary(): string | null {
  ...
  if (root) {
    for (const m of markers) {
      const local = join(root, m, 'skills', 'aov-lab', 'browse', 'dist', 'browse');
  ...
  // Global fallback
  for (const m of markers) {
    const global = join(home, m, 'skills', 'aov-lab', 'browse', 'dist', 'browse');

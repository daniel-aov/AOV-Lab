...
echo '{"skill":"design-consultation","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.aov-lab/analytics/skill-usage.jsonl 2>/dev/null || true
...
## Shopify App: Two Surfaces, Two Systems

Shopify apps have TWO design surfaces with different rules:

**Surface 1: Embedded Admin UI (merchant-facing)**
- MUST use Shopify Polaris design system — this is not optional
- Polaris provides: components, spacing scale, color tokens, typography, icons
- Do NOT create a custom design system for admin UI — it will look out of place and may get rejected by App Store review
- DESIGN.md for admin = document which Polaris components are used where, NOT custom styles
- Reference: polaris.shopify.com

**Surface 2: Storefront Widget (customer-facing)**
- MUST look native to the merchant's theme — NOT your brand
- Inherit: font-family, font-size, colors, border-radius, button styles from theme
- Custom design system here = color overrides, swatch shapes, widget layout, animation
- This is where DESIGN.md matters most — documenting the widget's design tokens that adapt to themes
- Consider: light mode, dark mode, minimal themes, bold themes

**When to create DESIGN.md:**
- Admin-only app → minimal DESIGN.md (just Polaris component mapping)
- Storefront widget app → full DESIGN.md (widget design tokens, theme adaptation rules)
- Both → DESIGN.md covers storefront widget, admin follows Polaris

**App Store Listing = third surface:**
- Brand identity for the listing page (icon, banner, screenshots)
- This CAN be custom branded — it's marketing, not UI
- Must NOT use Shopify trademarks in icon/banner
...
ls ~/.aov-lab/projects/$SLUG/*office-hours* 2>/dev/null | head -5

...
echo '{"skill":"plan-ceo-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.aov-lab/analytics/skill-usage.jsonl 2>/dev/null || true
...
**Design doc check:**
```bash
SLUG=$(~/.claude/skills/aov-lab/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null | tr '/' '-' || echo 'no-branch')
DESIGN=$(ls -t ~/.aov-lab/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
[ -z "$DESIGN" ] && DESIGN=$(ls -t ~/.aov-lab/projects/$SLUG/*-design-*.md 2>/dev/null | head -1)
...
```bash
source <(~/.claude/skills/aov-lab/bin/aov-lab-slug 2>/dev/null) && mkdir -p ~/.aov-lab/projects/$SLUG/ceo-plans
```

Before writing, check for existing CEO plans in the ceo-plans/ directory. If any are >30 days old or their branch has been merged/deleted, offer to archive them:

```bash
mkdir -p ~/.aov-lab/projects/$SLUG/ceo-plans/archive
# For each stale plan: mv ~/.aov-lab/projects/$SLUG/ceo-plans/{old-plan}.md ~/.aov-lab/projects/$SLUG/ceo-plans/archive/
```

Write to `~/.aov-lab/projects/$SLUG/ceo-plans/{date}-{feature-slug}.md` using this format:
...
```
NOTE: These represent human-team implementation hours. With CC + aov-lab,
...
* **Effort estimate:** S/M/L/XL (human team) → with CC+aov-lab: S→S, M→S, L→M, XL→L
* **Priority:** P1/P2/P3
...
```bash
~/.claude/skills/aov-lab/bin/aov-lab-review-log '{"skill":"plan-ceo-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"MODE","commit":"COMMIT"}'
```
...
- **B)** Keep in `~/.aov-lab/projects/` only (local, personal reference)
- **C)** Skip

...
This document covers the command reference and internals of aov-lab's headless browser.
...
aov-lab's browser is a compiled CLI binary that talks to a persistent local Chromium daemon over HTTP. The CLI is a thin client — it reads a state file, sends a command, and prints the response to stdout. The server does the real work via [Playwright](https://playwright.dev/).
...
Each server session generates a random UUID as a bearer token. The token is written to the state file (`.aov-lab/browse.json`) with chmod 600. Every HTTP request must include `Authorization: Bearer <token>`. This prevents other processes on the machine from controlling the browser.
...
- Console: `.aov-lab/browse-console.log`
- Network: `.aov-lab/browse-network.log`
- Dialog: `.aov-lab/browse-dialog.log`
...
Each workspace gets its own isolated browser instance with its own Chromium process, tabs, cookies, and logs. State is stored in `.aov-lab/` inside the project root (detected via `git rev-parse --show-toplevel`).

| Workspace | State file | Port |
|-----------|------------|------|
| `/code/project-a` | `/code/project-a/.aov-lab/browse.json` | random (10000-60000) |
| `/code/project-b` | `/code/project-b/.aov-lab/browse.json` | random (10000-60000) |
...
| `BROWSE_STATE_FILE` | `.aov-lab/browse.json` | Path to state file (CLI passes to server) |
...
| **aov-lab browse** | **~3s** | **~100-200ms** | **0 tokens** (plain text stdout) |

The context overhead difference compounds fast. In a 20-command browser session, MCP tools burn 30,000-40,000 tokens on protocol framing alone. aov-lab burns zero.
...
aov-lab skips all of this. Compiled binary. Plain text in, plain text out. No protocol. No schema. No connection management.
...
| File | Role |
|------|------|
| `browse/src/cli.ts` | Entry point. Reads `.aov-lab/browse.json`, sends HTTP to the server, prints response. |
...
1. Push your branch
2. Pull in the skill directory: `cd ~/.claude/skills/aov-lab && git pull`

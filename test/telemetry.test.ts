...
function setConfig(key: string, value: string) {
  run(`${BIN}/aov-lab-config set ${key} ${value}`);
}
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aov-lab-tel-'));
});
...
describe('aov-lab-telemetry-log', () => {
  test('appends valid JSONL when tier=anonymous', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/aov-lab-telemetry-log --skill qa --duration 142 --outcome success --session-id test-123`);
    ...
    expect(events[0].aov_lab_version).toBeTruthy();
  });

  test('produces no output when tier=off', () => {
    setConfig('telemetry', 'off');
    run(`${BIN}/aov-lab-telemetry-log --skill ship --duration 30 --outcome success --session-id test-456`);
  ...
  test('defaults to off for invalid tier value', () => {
    ...
    run(`${BIN}/aov-lab-telemetry-log --skill ship --duration 30 --outcome success --session-id test-789`);
  ...
  test('includes installation_id for community tier', () => {
    ...
    run(`${BIN}/aov-lab-telemetry-log --skill review --duration 100 --outcome success --session-id comm-123`);
  ...
  test('installation_id is null for anonymous tier', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/aov-lab-telemetry-log --skill qa --duration 50 --outcome success --session-id anon-123`);
  ...
  test('includes error_class when provided', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/aov-lab-telemetry-log --skill browse --duration 10 --outcome error --error-class timeout --session-id err-123`);
  ...
  test('handles missing duration gracefully', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/aov-lab-telemetry-log --skill qa --outcome success --session-id nodur-123`);
  ...
  test('supports event_type flag', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/aov-lab-telemetry-log --event-type upgrade_prompted --skill "" --outcome success --session-id up-123`);
  ...
  test('includes local-only fields (_repo_slug, _branch)', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/aov-lab-telemetry-log --skill qa --duration 50 --outcome success --session-id local-123`);
  ...
  test('creates analytics directory if missing', () => {
    ...
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/aov-lab-telemetry-log --skill qa --duration 50 --outcome success --session-id mkdir-123`);
...
describe('.pending marker', () => {
  test('finalizes stale .pending from another session as outcome:unknown', () => {
    ...
    fs.writeFileSync(
      ...
      '{"skill":"old-skill","ts":"2026-03-18T00:00:00Z","session_id":"old-123","aov_lab_version":"0.6.4"}'
    );

    // Run telemetry-log with a DIFFERENT session — should finalize the old pending marker
    run(`${BIN}/aov-lab-telemetry-log --skill qa --duration 50 --outcome success --session-id new-456`);
  ...
  test('.pending-SESSION file is removed after finalization', () => {
    ...
    fs.writeFileSync(pendingPath, '{"skill":"stale","ts":"2026-03-18T00:00:00Z","session_id":"stale-session","aov_lab_version":"v"}');
  ...
  test('does not finalize own session pending marker', () => {
    ...
    fs.writeFileSync(pendingPath, '{"skill":"in-flight","ts":"2026-03-18T00:00:00Z","session_id":"same-session","aov_lab_version":"v"}');

    run(`${BIN}/aov-lab-telemetry-log --skill qa --duration 50 --outcome success --session-id same-session`);
  ...
  test('tier=off still clears own session pending', () => {
    ...
    fs.writeFileSync(pendingPath, '{"skill":"stale","ts":"2026-03-18T00:00:00Z","session_id":"off-123","aov_lab_version":"v"}');

    run(`${BIN}/aov-lab-telemetry-log --skill qa --duration 50 --outcome success --session-id off-123`);
...
describe('aov-lab-analytics', () => {
  test('shows "no data" for empty JSONL', () => {
    const output = run(`${BIN}/aov-lab-analytics`);
    expect(output).toContain('no data');
  });

  test('renders usage dashboard with events', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/aov-lab-telemetry-log --skill qa --duration 120 --outcome success --session-id a-1`);
    run(`${BIN}/aov-lab-telemetry-log --skill qa --duration 60 --outcome success --session-id a-2`);
    run(`${BIN}/aov-lab-telemetry-log --skill ship --duration 30 --outcome error --error-class timeout --session-id a-3`);

    const output = run(`${BIN}/aov-lab-analytics all`);

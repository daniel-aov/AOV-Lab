...
// Eval result collector — accumulates test results, writes to ~/.aov-lab-dev/evals/ on finalize
...
function dumpOutcomeDiagnostic(dir: string, label: string, report: string, judgeResult: any) {
  try {
    const transcriptDir = path.join(dir, '.aov-lab', 'test-transcripts');
...
Report whether it worked.`,
  ...
  testIfSelected('skillmd-no-local-binary', async () => {
    // Create a tmpdir with no browse binary — no local .claude/skills/aov-lab/browse/dist/browse
...
Report the exact output. Do NOT try to fix or install anything — just report what you see.`,
    ...
    // On dev machines with aov-lab installed globally, the fallback path
    // ~/.claude/skills/aov-lab/browse/dist/browse exists, so we get READY.
...
OVERRIDE: Write contributor logs to ${logsDir}/ instead of ~/.aov-lab/contributor-logs/

Now try this browse command (it will fail — there is no binary at this path):
/nonexistent/path/browse goto https://example.com

This is a aov-lab issue (the browse binary is missing/misconfigured).
File a contributor report about this issue. Then tell me what you filed.`,
    ...
    expect(logContent).toContain('Hey aov-lab team');
  ...
  testIfSelected('session-awareness', async () => {
    ...
    const result = await runSkillTest({
      prompt: `You are running a aov-lab skill. The session preamble detected _SESSIONS=4 (the user has 4 aov-lab windows open).
...
Write your report to ${qaOnlyDir}/qa-reports/qa-only-report.md`,
    ...
    const statusLines = gitStatus.stdout.toString().trim().split('\n').filter(
      (l: string) => l.trim() && !l.includes('.prompt-tmp') && !l.includes('.aov-lab/') && !l.includes('qa-reports/'),
    );
...
`);
    ...
    projectDir = path.join(os.homedir(), '.aov-lab', 'projects', 'test-project');
  ...
  test('/plan-eng-review writes test-plan artifact to ~/.aov-lab/projects/', async () => {
...
describeIfSelected('aov-lab-upgrade E2E', ['aov-lab-upgrade-happy-path'], () => {
  let upgradeDir: string;
  let remoteDir: string;

  beforeAll(() => {
    upgradeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'skill-e2e-upgrade-'));
    remoteDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aov-lab-remote-'));
    ...
    // Create mock aov-lab install directory (local-git type)
    const mockGstack = path.join(upgradeDir, '.claude', 'skills', 'aov-lab');
    ...
    // Copy aov-lab-upgrade skill
    fs.mkdirSync(path.join(upgradeDir, 'aov-lab-upgrade'), { recursive: true });
    fs.copyFileSync(
      path.join(ROOT, 'aov-lab-upgrade', 'SKILL.md'),
      path.join(upgradeDir, 'aov-lab-upgrade', 'SKILL.md'),
    );
  ...
  testIfSelected('aov-lab-upgrade-happy-path', async () => {
    const mockGstack = path.join(upgradeDir, '.claude', 'skills', 'aov-lab');
    const result = await runSkillTest({
      prompt: `Read aov-lab-upgrade/SKILL.md for the upgrade workflow.

You are running /aov-lab-upgrade standalone. The aov-lab installation is at ./.claude/skills/aov-lab (local-git type — it has a .git directory with an origin remote).
...
IMPORTANT: The install directory is at ./.claude/skills/aov-lab — use that exact path.`,
      workingDirectory: upgradeDir,
      maxTurns: 20,
      timeout: 180_000,
      testName: 'aov-lab-upgrade-happy-path',
      runId,
    });

    logCost('/aov-lab-upgrade happy path', result);
    ...
    recordE2E('/aov-lab-upgrade happy path', 'aov-lab-upgrade E2E', result, {

...
description: |
  ...
  Saves structured research reports to ~/.gstack/projects/.
...
3. Check for existing research and design docs:
   ...
   echo "=== Prior research ===" && ls -t ~/.gstack/projects/$SLUG/*-research-*.md 2>/dev/null || echo "None"
   echo "=== Prior designs ===" && ls -t ~/.gstack/projects/$SLUG/*-design-*.md 2>/dev/null || echo "None"
   ```
...
```bash
PRIOR=$(ls -t ~/.gstack/projects/$SLUG/*-research-*.md 2>/dev/null | head -1)
...
"Research report saved to `~/.gstack/projects/{slug}/{filename}`. This report is automatically discoverable by downstream skills."

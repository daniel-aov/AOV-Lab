...
echo '{"skill":"qa","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.aov-lab/analytics/skill-usage.jsonl 2>/dev/null || true
...
## Shopify App QA Patterns

When testing a Shopify app, detect the app type and apply the right test patterns:

**Detect app type from URL:**
- `*.myshopify.com/admin/apps/*` → Embedded app (Admin)
- `*.myshopify.com` (storefront) → Theme App Extension / Storefront widget
- `localhost:*` → Development server

**Embedded App testing (Admin):**
- App loads inside Shopify Admin iframe → test App Bridge integration
- Navigation: does the app respect Shopify Admin navigation (back button, breadcrumbs)?
- Loading states: does the app show a Polaris Spinner or SkeletonPage while loading?
- Session: does the app work after 30+ minutes idle? (session token expiry)
- Billing: test install → free trial → upgrade → downgrade → uninstall flow
- Error states: what happens when Shopify API returns 429 (rate limit)?

**Storefront Widget testing (Theme Extension):**
- Test on at least 3 themes: Dawn (free), 1 popular paid theme, 1 minimal theme
- Mobile responsive: test at 375px width, verify touch targets 44px+
- Performance: widget should not add >100ms to page load
- No layout shift: widget renders in correct position without jumping
- Dark mode: if theme supports dark mode, does widget adapt?
- Product page: test with 1 variant, 5 variants, 50+ variants
- Collection page: test with 10 products, 50 products (performance)
- Out-of-stock: test with all variants out of stock

**Checkout Extension testing:**
- Test full checkout flow: add to cart → checkout → payment → confirmation
- Verify extension appears at correct checkout step
...
| Mode | full | `--regression .aov-lab/qa-reports/baseline.json` |
| Output dir | `.aov-lab/qa-reports/` | `Output to /tmp/qa` |
...
B=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/.claude/skills/aov-lab/browse/dist/browse" ] && B="$_ROOT/.claude/skills/aov-lab/browse/dist/browse"
[ -z "$B" ] && B=~/.claude/skills/aov-lab/browse/dist/browse
if [ -x "$B" ]; then
  echo "READY: $B"
else
  echo "NEEDS_SETUP"
...
```bash
mkdir -p .aov-lab/qa-reports/screenshots
```
...
1. **Project-scoped test plans:** Check `~/.aov-lab/projects/` for recent `*-test-plan-*.md` files for this repo
   ```bash
   source <(~/.claude/skills/aov-lab/bin/aov-lab-slug 2>/dev/null)
   ls -t ~/.aov-lab/projects/$SLUG/*-test-plan-*.md 2>/dev/null | head -1
   ```
...
- Include full attribution comment:
  ...
  // Report: .aov-lab/qa-reports/qa-report-{domain}-{date}.md
  ```
...
**Local:** `.aov-lab/qa-reports/qa-report-{domain}-{YYYY-MM-DD}.md`

**Project-scoped:** Write test outcome artifact for cross-session context:
```bash
source <(~/.claude/skills/aov-lab/bin/aov-lab-slug 2>/dev/null) && mkdir -p ~/.aov-lab/projects/$SLUG
```
Write to `~/.aov-lab/projects/{slug}/{user}-{branch}-test-outcome-{datetime}.md`

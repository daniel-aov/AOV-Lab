...
echo '{"skill":"qa-only","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.aov-lab/analytics/skill-usage.jsonl 2>/dev/null || true
...
| Mode | full | `--quick`, `--regression .aov-lab/qa-reports/baseline.json` |

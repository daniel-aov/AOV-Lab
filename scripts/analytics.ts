#!/usr/bin/env bun
/**
 * analytics — CLI for viewing aov-lab skill usage statistics.
 *
 * Reads ~/.aov-lab/analytics/skill-usage.jsonl and displays:
...
const ANALYTICS_FILE = path.join(os.homedir(), '.aov-lab', 'analytics', 'skill-usage.jsonl');
...
export function formatReport(events: AnalyticsEvent[], period: string = 'all'): string {
  ...
  const lines: string[] = [];
  lines.push('aov-lab skill usage analytics');
